...
- 保存目标语言、是否显示原文、文字覆盖模式、转写接口和 API Key。
- 文字覆盖模式：把 OCR 识别到的屏幕文字块翻译后显示到原位置附近。
...
- 将文字覆盖从最多 6 个文字块升级为更细的逐行覆盖、遮挡避让和稳定跟踪。
- 做正式签名 APK 或安装包发布流程。
...
GitHub CLI 已登录过，但之前推送遇到网络或凭据问题。Android APK 构建完成后需要继续重试推送。
